...
namespace Greg.Xrm.Command.Commands.Projects
{
	...
	public class InitProjectCommand : ICanProvideUsageExample
	{
		...
		[Option("solution", "s", Order = 2, HelpText = "The default solution to use to store the customizations for the current project. It overrides any solution set via `pacx solution setDefault`. If not provided, the default solution for the selected auth profile is used.")]
