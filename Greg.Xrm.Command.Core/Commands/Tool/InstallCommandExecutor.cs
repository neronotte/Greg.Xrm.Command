...
{
	public class InstallCommandExecutor(
			IOutput output,
			IStorage storage) : ICommandExecutor<InstallCommand>
	{
		private readonly Logger logger = new(output);
		...
		private bool ExtractPackageContentToFolder(PackageArchiveReader packageReader, string rootPackageId, NuGetFramework? bestSupportedFramework, string[] validFiles)
		{
			...
			var packageVersion = nuspecReader.GetVersion().ToString();
		...
		private void PrepareDestinationFolder(string packageId)
		{
			...
			var currentPluginFolder = pluginStorageFolder.GetDirectories().FirstOrDefault(d => d.Name.Equals(packageId, StringComparison.OrdinalIgnoreCase));
			currentPluginFolder?.Delete(true);

			pluginStorageFolder.CreateSubdirectory(packageId);
		}

		private NuGetFramework? GetBestSupportedFramework(IEnumerable<NuGetFramework> frameworks)
		{
			var projectFramework = NuGetFramework.Parse(AppContext.TargetFrameworkName ?? string.Empty);
		...
		private bool ValidatePackage(PackageArchiveReader packageReader, string rootPackageId)
		{
			...
			if (!isValid)
			{
				output.WriteLine($"Package <{rootPackageId}> is not a valid PACX plugin. A valid PACX plugin must have a dependency on <Greg.Xrm.Command.Interfaces>.", ConsoleColor.Red);
				return false;
			}
		...
		private async Task<(bool, PackageIdentity? packageIdentity, bool)> ValidatePackage(SourceRepository repository, string packageId, string packageVersion, SourceCacheContext cacheContext, CancellationToken cancellationToken)
		{
			...
			{
				var canParseVersion = SemanticVersion.TryParse(packageVersion, out SemanticVersion? parsedVersion);
				if (!canParseVersion)
				{
		...
		private async Task<bool> ExtractPackageStream(Stream? packageStream, string packageId, string path, CancellationToken cancellationToken)
		{
			...
			using (var packageReader = new PackageArchiveReader(packageStream))
			{
				...
				{
					validFiles = [.. validFiles.Where(f => Path.GetExtension(f).ToLowerInvariant().EndsWith(".dll"))];
				}
		...
		private async Task<CommandResult> DownloadPluginFromFile(InstallCommand command, CancellationToken cancellationToken)
		{
			...
			await packageStream.CopyToAsync(infoStream, cancellationToken);
		...
		class Logger(IOutput output) : LoggerBase
		{
			public override void Log(ILogMessage message)
			{
				var color = message.Level switch
				{
					...
					LogLevel.Minimal => ConsoleColor.Gray,
					LogLevel.Verbose => ConsoleColor.Gray,
					LogLevel.Warning => ConsoleColor.Yellow,
					_ => ConsoleColor.White,
				};

				output.WriteLine(message.Message, color);
			}
