...
{
	[Command("plugin", "step", "enable", HelpText = "Reactivates a plugin step registration that has previously been disabled.")]
	[Alias("plugin", "step", "activate")]
	public class EnableCommand : IValidatableObject, ICanProvideUsageExample
	{
		...
		{
			writer.WriteParagraph("To re-enable a plugin step, you have two ways:");
			writer.WriteList(
