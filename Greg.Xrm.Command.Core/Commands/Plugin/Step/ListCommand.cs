...
{
	[Command("plugin", "step", "list", HelpText = "Lists plugin steps associated with a plugin assembly, plugin type, table, or solution.")]
	public class ListCommand : ICanProvideUsageExample, IValidatableObject
	{
		...
		[Option("showInternalPlugins", "all", Order = 5, HelpText = "Include internal system plugin steps (all stages). By default, only user-manageable stages are shown (PreValidation, PreOperation, PostOperation).", DefaultValue = false)]
