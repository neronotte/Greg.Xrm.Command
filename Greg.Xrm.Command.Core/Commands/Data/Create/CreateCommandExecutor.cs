...
namespace Greg.Xrm.Command.Commands.Data.Create
{
	...
	) : ICommandExecutor<CreateCommand>
	{
		public async Task<CommandResult> ExecuteAsync(CreateCommand command, CancellationToken cancellationToken)
		{
			...
			{
				output.WriteLine("FAILED", ConsoleColor.Red);
				return CommandResult.Fail($"Error creating record: {ex.Message}", ex);
			}
			catch (Exception ex) when (ex is not OperationCanceledException)
			{
				output.WriteLine("FAILED", ConsoleColor.Red);
				return CommandResult.Fail($"Unexpected error creating record: {ex.Message}", ex);
			}
