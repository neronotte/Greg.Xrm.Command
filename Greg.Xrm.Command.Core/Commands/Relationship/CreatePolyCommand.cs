...
namespace Greg.Xrm.Command.Commands.Relationship
{
	...
	public class CreatePolyCommand : ICanProvideUsageExample
	{
		...
		[Option("parents", "p", Order = 2, HelpText = "A comma or pipe separated list of entities that will act as parent for the relationship")]
		[Required]
		public string Parents { get; set; } = string.Empty;

		[Option("lookupDisplayName", "ldn", Order = 3, HelpText = "The display name of the lookup attribute.")]
		[Required]
		public string LookupAttributeDisplayName { get; set; } = string.Empty;

		[Option("lookupSchemaName", "lsn", Order = 4, HelpText = "The schema name of the lookup attribute. If not specified, it is inferred by the display name.")]
		public string? LookupAttributeSchemaName { get; set; }

		[Option("requiredLevel", "r", Order = 5, HelpText = "The required level of the lookup attribute.")]
		public AttributeRequiredLevel RequiredLevel { get; set; } = AttributeRequiredLevel.None;



		[Option("relNameSuffix", "suff", Order = 6, HelpText = "The suffix to append to the relationship name. If not provided, will be set equal to the display name of the lookup attribute (only letters, numbers, or underscores, lowercase).")]
		public string? RelationshipNameSuffix { get; set; }



		[Option("cascadeAssign", "caass", Order = 20, HelpText = "The behavior to apply to child records when the parent record is assigned to another owner\n(values: Cascade, Active, UserOwned, NoCascade)\n(default: NoCascade)", SuppressValuesHelp = true)]
		public CascadeType? CascadeAssign { get; set; }

		[Option("cascadeArchive", "caarc", Order = 21, HelpText = "The behavior to apply to child records when the parent record is archived\n(not available via UI)\n(default: NoCascade)", SuppressValuesHelp = true)]
		public CascadeType? CascadeArchive { get; set; }

		[Option("cascadeShare", "cas", Order = 22, HelpText = "The behavior to apply to child records when the parent record is shared\n(values: Cascade, Active, UserOwned, NoCascade)\n(default: NoCascade)", SuppressValuesHelp = true)]
		public CascadeType? CascadeShare { get; set; }

		[Option("cascadeUnshare", "cau", Order = 23, HelpText = "The behavior to apply to child records when the parent record is unshared\n(values: Cascade, Active, UserOwned, NoCascade)\n(default: NoCascade)", SuppressValuesHelp = true)]
		public CascadeType? CascadeUnshare { get; set; }

		[Option("cascadeDelete", "cad", Order = 24, HelpText = "The behavior to apply when the parent record is deleted\n(values: Restrict, RemoveLink)\n(default: Restrict)", DefaultValue = CascadeType.Restrict, SuppressValuesHelp = true)]
		public CascadeType? CascadeDelete { get; set; } = CascadeType.Restrict;

		[Option("cascadeMerge", "cam", Order = 25, HelpText = "The behavior to apply to child records when the parent record is merged to another one\n(not available via UI)\n(default: NoCascade)", SuppressValuesHelp = true)]
		public CascadeType? CascadeMerge { get; set; }

		[Option("cascadeReparent", "car", Order = 26, HelpText = "The behavior to apply to child records when the parent record is reparented\n(values: Cascade, Active, UserOwned, NoCascade)\n(default: NoCascade)", SuppressValuesHelp = true)]
		...
		[Option("menuBehavior", "m", Order = 27, HelpText = "Indicates how the child entity is displayed in the parent navbar", DefaultValue = AssociatedMenuBehavior.DoNotDisplay)]
		public AssociatedMenuBehavior MenuBehavior { get; set; } = AssociatedMenuBehavior.DoNotDisplay;

		[Option("menuLabel", "ml", Order = 28, HelpText = "Associated menu label. To be specified only if the menuBehavior arg is set to UseLabel", DefaultValue = null)]
		public string? MenuLabel { get; set; }

		[Option("menuGroup", "mg", Order = 29, HelpText = "Associated menu group. To be specified only if the menuBehavior arg is set to UseLabel or UseCollectionName", DefaultValue = AssociatedMenuGroup.Details)]
		public AssociatedMenuGroup MenuGroup { get; set; } = AssociatedMenuGroup.Details;

		[Option("menuOrder", "mo", Order = 30, HelpText = "Associated menu order. To be specified only if the menuBehavior arg is set to UseLabel or UseCollectionName", DefaultValue = 10000)]
		...
		[Option("solution", "s", Order = 31, HelpText = "The name of the unmanaged solution to which you want to add this relationship.")]
